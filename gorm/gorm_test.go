...
func OpenConnection() *gorm.DB {
	dialect := mysql.Open("root:@tcp(localhost:3306)/db_gorm?charset=utf8mb4&parseTime=True&loc=Local")
...
func TestExecuteSQL(t *testing.T) {
	err := db.Exec("insert into sample(id, name) values (?, ?)", "1", "Ivan").Error
	assert.Nil(t, err)
...
func TestRawSQL(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "Ivan", sample.Name)
