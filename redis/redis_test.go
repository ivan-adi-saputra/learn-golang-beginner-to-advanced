...
func TestString(t *testing.T) {
	client.SetEx(ctx, "name", "Ivan Adi", 3*time.Second)

	result, err := client.Get(ctx, "name").Result()
	assert.Nil(t, err)
	assert.Equal(t, "Ivan Adi", result)
...
func TestList(t *testing.T) {
	client.RPush(ctx, "names", "Ivan")
	client.RPush(ctx, "names", "Adi")
	client.RPush(ctx, "names", "Saputra")

	assert.Equal(t, "Ivan", client.LPop(ctx, "names").Val())
	assert.Equal(t, "Adi", client.LPop(ctx, "names").Val())
...
func TestSet(t *testing.T) {
	client.SAdd(ctx, "students", "Ivan")
	client.SAdd(ctx, "students", "Ivan")
	client.SAdd(ctx, "students", "Adi")
	client.SAdd(ctx, "students", "Adi")
	client.SAdd(ctx, "students", "Saputra")
	client.SAdd(ctx, "students", "Saputra")

	assert.Equal(t, int64(3), client.SCard(ctx, "students").Val())
	assert.Equal(t, []string{"Ivan", "Adi", "Saputra"}, client.SMembers(ctx, "students").Val())
}

func TestSortedSet(t *testing.T) {
	client.ZAdd(ctx, "scores", redis.Z{Score: 100, Member: "Ivan"})
	client.ZAdd(ctx, "scores", redis.Z{Score: 85, Member: "Budi"})
	client.ZAdd(ctx, "scores", redis.Z{Score: 95, Member: "Joko"})

	assert.Equal(t, []string{"Budi", "Joko", "Ivan"}, client.ZRange(ctx, "scores", 0, -1).Val())

	assert.Equal(t, "Ivan", client.ZPopMax(ctx, "scores").Val()[0].Member)
...
func TestHash(t *testing.T) {
	...
	client.HSet(ctx, "user:1", "name", "Ivan")
	client.HSet(ctx, "user:1", "email", "Ivan@example.com")
...
func TestHyperLogLog(t *testing.T) {
	client.PFAdd(ctx, "visitors", "Ivan", "Adi", "Saputra")
	client.PFAdd(ctx, "visitors", "Ivan", "budi", "joko")
...
func TestPipeline(t *testing.T) {
	_, err := client.Pipelined(ctx, func(pipeliner redis.Pipeliner) error {
		pipeliner.SetEx(ctx, "name", "Ivan", 5*time.Second)
		pipeliner.SetEx(ctx, "address", "Indonesia", 5*time.Second)
		return nil
	})
	assert.Nil(t, err)

	assert.Equal(t, "Ivan", client.Get(ctx, "name").Val())
